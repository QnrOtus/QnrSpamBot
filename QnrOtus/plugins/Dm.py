...
async def _(e):   
    ...
    if e.sender_id in SUDO_USERS:
        ...
        QnrOtus = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        smex = await e.get_reply_message()
        if len(QnrOtus) == 2:
            user = str(QnrOtus[0])
            a = await e.client.get_entity(user)
            g = a.id
            if int(g) in QnrOtusX:
                text = f"I can't Dm to @QnrOtus's Owner"
...
async def dmraid(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(QnrOtus) == 2:
            ...
            g = a.id
            if int(g) in QnrOtusX:
                text = f"I can't raid on @QnrOtus's Owner"
                await e.reply(text, parse_mode=None, link_preview=None )
            elif int(g) in SUDO_USERS:
                text = f"This guy is a sudo user."
                await e.reply(text, parse_mode=None, link_preview=None )
            else:
                counter = int(QnrOtus[0])
